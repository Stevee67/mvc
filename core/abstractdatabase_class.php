...
abstract class AbstractDataBase{
    ...
    protected function __construct($db_host, $db_user, $db_password, $db_name, $sq, $prefix, $local){
        $this->mysqli = @new mysqli($db_host, $db_user, $db_password, $db_name);
        if($this->mysqli->connect_errno) exit("Помилка підключення з базою даних");
        $this->sq = $sq;
        $this->prefix = $prefix;
        $this->mysqli("SET lc_time_names = '$local'");
    ...
    public function insert($table_name, $row){
        ...
        $fields .= ")";
        $values .= ")";
        $query = "INSERT INTO `$table_name` $fields $value";
    ...
    public function update($table_name, $row, $where = false, $params = array()){
        ...
        if($where){
            $params = array_marge($params_add, $params);
            $query .= " WHERE $where";
        }
    ...
    public function __destruct(){
        if(($this->mysqli) && (!$this->mysqli->errno)) $this->mysqli->close();
    }
}